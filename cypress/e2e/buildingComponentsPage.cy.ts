...
describe(
  ...
  () => {
        ...
        .mockGraphqlOps<OPERATIONS, 'updateProjectSourceInterpretation', OPERATIONS['updateProjectSourceInterpretation']>(
          'updateProjectSourceInterpretation',
          {
            resolver: ({ variables: { id, type, interpretation } }) => ({
              updateProjectSource: { id: '3481d0bb-a0b5-49fa-bdea-27a4253cff0a', interpretation: {
    ...
    it('should add source directly from components page', () => {
      ...
      cy.get('.MuiPaper-root > .MuiList-root').first().click()
      cy.get('input[type="file"]').attachFile('../fixtures/test.csv')
      cy.get('[data-testid="add-project-source-button"] > .MuiTypography-root').click()
      cy.get('[data-testid="sources-elements-table"] > .MuiDataGrid-root > .MuiDataGrid-main > .MuiDataGrid-columnHeaders > .MuiDataGrid-columnHeadersInner > .css-yrdy0g-MuiDataGrid-columnHeaderRow > [tabindex="0"] > .MuiDataGrid-columnHeaderDraggableContainer > .MuiDataGrid-columnHeaderTitleContainer > .MuiDataGrid-columnHeaderTitleContainerContent > div')
        .within(() => {
          cy.get('input[type=checkbox]').first().click()
      });
      cy.get('.MuiDialogActions-root').then((buttons)=>{
        console.log('buttons',buttons)
      })
      cy.get('[data-testid="source-interpretation-button-done"]').click()
    })

    it('should take user back to add elements from source pop-up', () => {
      expandCategories()
      cy.get(`[data-id=${existingNestedCategoryWithElements.elements[0].id}]`)
      cy.get('[data-testid="ControlPointDuplicateOutlinedIcon"]').click()
      // cy.get('[data-testid=addMultipleSourceElementsButton]').click()
      cy.get('[data-testid=add-source-icon-button]').click()
      cy.get('[data-testid="cancel-project-source-button"] > .MuiTypography-root').click()
      cy.get('.MuiDialogContent-root').contains('Category Name')
    })
  },
)
